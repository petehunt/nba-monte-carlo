...
You can start writing assets in `dagster_nba_monte_carlo/assets/`. The assets are automatically loaded into the Dagster repository as you define them.
...
Tests are in the `dagster_nba_monte_carlo_tests` directory and you can run tests using `pytest`:

```bash
pytest dagster_nba_monte_carlo_tests
```
